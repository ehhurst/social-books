...
app = Flask(__name__)
CORS(app, supports_credentials=True, resources={r"/*": {"origins": ["http://localhost:5173", "http://34.238.53.95:5173"]}})
...
def user_contests_options(username):
    ...
    response.headers["Access-Control-Allow-Origin"] = "http://34.238.53.95:5173"
...
def contest_deadline(contest_name):
    ...
    dt_dead = datetime.strptime(deadline_time, "%Y-%m-%d")
...
def contest_checklist(contest_name):
    ...
    cursor.execute(query, (competitor[0], contest_name))
